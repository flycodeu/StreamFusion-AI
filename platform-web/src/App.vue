...
async function checkHealth(): Promise<void> {
  ...
  detail.value = ''
  try {
    const response = await fetch('/actuator/health', { signal: AbortSignal.timeout(5000) })
...
<template>
  <div class="shell">
    <header><span class="brand-mark" aria-hidden="true">SF</span><span>StreamFusion <strong>AI</strong></span><span class="phase">P0 / 工程初始化</span></header>
    <main>
      ...
      <section class="health" aria-labelledby="health-title">
        <div class="section-heading"><h2 id="health-title">后端连接</h2><ElButton :loading="loading" @click="checkHealth">重新检查</ElButton></div>
        <div role="status" aria-live="polite">
          <p class="status"><span class="dot" :class="{ ready: status === '运行正常', failed: status === '连接失败' }"></span>{{ status }}</p>
